...
class Uniformer_DataSet(data.Dataset):
    ...
    def get(self, record, indices):
        ...
        # focal_list1 = ['F-15','F0','F15']
        # focal_list2 = ['F-25','F0','F25']
        focal_list1 = ['F-30','F-15','F0','F15','F30']
        focal_list2 = ['F-50','F-25','F0','F25','F50']
        ...
        for focal in focal_list:
            for seg_ind in indices:
                ...
                images.append(self.transform(seg_imgs[0]).unsqueeze(1))#(C,1,H,W)
        #! 增加针对视频的数据增强
        ...
        # process_data = process_data.reshape(len(focal_list),-1,process_data.size(-2),process_data.size(-1))
        ...
        return process_data, record.label, num_pos, embIndex, femaleAge, maleAge

    def __len__(self):
        return len(self.video_list)
    
    
class Uniformer_Feature_DataSet(data.Dataset):
    def __init__(self, root_path, list_file,table_path=None,
                 num_segments=32, new_length=1, modality='RGB',
                 image_tmpl='img_{:05d}.jpg', transform=None, group_transform=None,
                 random_shift=True, test_mode=False,
    ...
    def __getitem__(self, index):
        ...
        # check this is a legit video folder
        tmp_dir = os.path.join('/data2/liangzhijia/datasets/embryo_feature','/'.join(record.path.split('/')[5:]))
        focal_file = [file for root,dirs,files in os.walk(tmp_dir) for file in files]
        feature_lst = []
        for focal in focal_file:
            feature = torch.load(os.path.join(tmp_dir,focal),map_location='cpu')
            feature_lst.append(feature.unsqueeze(0))
        focal_feature = torch.cat(feature_lst, dim=0)
        embIndex, femaleAge, maleAge = self.new_get_patient_data(record)    
        
        return focal_feature, record.label, self.num_segments, embIndex, femaleAge, maleAge
...
if __name__ == "__main__":
    dataset = Uniformer_DataSet(
        "",
        "/data2/liangzhijia/Blastocyst/Uniformerv2_ART/data_opt/test_data.txt",
    ...
    for imgs, lable, num_pos, embIndex, femaleAge, maleAge in dataset:
        print(imgs.shape)
        print(lable)
        print(num_pos)
        print(embIndex)
